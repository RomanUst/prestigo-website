...
describe('DriverAssignmentSection — Phase 53 prop evolution', () => {
  ...
  it('renders the DRIVER section for confirmed bookings', async () => {
    ...
    await waitFor(() => expect(screen.getByText('DRIVER')).toBeDefined())
  })
  ...
  it('invokes onAssigned("assigned") after a successful POST (D-06)', async () => {
    ...
    // Sequence: GET drivers+assignment → POST assign → GET assignment refresh
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({ drivers: [{ id: driverId, name: 'Test Driver' }], assignment: null }) })
  ...
  it('does NOT invoke onAssigned when POST fails', async () => {
    ...
    // wait for the error mode to settle
