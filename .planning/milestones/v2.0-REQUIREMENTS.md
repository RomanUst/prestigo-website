# Requirements Archive: v2.0 Blacklane-style Booking + Customer Accounts
...
- [x] **ACCT-04**: New bookings made by a logged-in customer are linked to their `user_id` via a nullable FK on `bookings`; anonymous/guest bookings remain valid and unaffected. _Scaffolded in Phase 57; wired end-to-end in Phase 60 (commit 921a15b: create-payment-intent passes userId, buildBookingRow stores it, migration 049 adds RLS)._
...
- [ ] **BOOK-06**: Booking-method step lets the user choose "Book for myself (account)" or "Book as guest"; corporate accounts also get "Book for a guest" — _deferred to v2.1_
- [x] **BOOK-07**: A logged-in customer's contact details are pre-filled in the passenger step
- [x] **BOOK-08**: Guest checkout is available at every stage — sign-in is always optional, never blocking
...
- [x] **TRACK-02**: All Meta Pixel + CAPI events preserved (`InitiateCheckout`, `AddPaymentInfo`, `Purchase`), including `eventId` deduplication — _code verified in Phase 61; live round-trip blocked by third-party (consent + payment required)_
...
- [x] **TRACK-04**: GA4 `login` and `sign_up` events fire on customer sign-in and registration — _code verified (Step3Auth.tsx fires gtag events); live round-trip blocked by third-party (OTP delivery required)_
...
| ACCT-04 | Phase 57 (scaffolded) + Phase 60 (wired) | Complete |
...
| BOOK-06 | — | Deferred → v2.1 |
| BOOK-07 | Phase 60 | Complete |
| BOOK-08 | Phase 60 | Complete |
| TRACK-01 | Phase 59 (verified Phase 61) | Complete |
| TRACK-02 | Phase 59 (verified Phase 61) | Complete (code; live blocked by 3rd party) |
| TRACK-03 | Phase 59 (verified Phase 61) | Complete |
| TRACK-04 | Phase 60 (verified Phase 61) | Complete (code; live blocked by 3rd party) |
| TRACK-05 | Phase 59 (verified Phase 61) | Complete |

**Coverage:** 26/26 v2.0 requirements mapped — no orphans, no duplicates. 25 Complete, 1 Deferred (BOOK-06 → v2.1). Each TRACK requirement has a primary owner phase; Phase 61 is end-to-end verification.

---

*Last updated: 2026-06-18 — Post-audit correction: ACCT-04 traceability fixed (Phase 60); BOOK-07/08, TRACK-02/04 marked complete; BOOK-06 deferred to v2.1.*
