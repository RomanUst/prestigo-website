...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-kutna-hora', locale)
  const route = await getRoutePrice('prague-kutna-hora')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-kutna-hora', {
    indexable: true,
    content: { kind: 'route', key: 'prague-kutna-hora' },
    locale,
  })
  return {
...
export default async function PragueKutnaHoraPage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-kutna-hora', {
            locale,
    ...
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0">
          <Image src="/photohero.jpg" alt="Kutná Hora — private chauffeur transfer from Prague to Kutná Hora" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} />
        </div>
                ...
                <div className="p-8 flex flex-col gap-6 flex-1">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between">
                      <span className="font-body font-light text-[11px] text-warmgrey tracking-[0.05em]">{t('vehicleFields.passengers')}</span>
                      <span className="font-body font-light text-[11px] text-offwhite">{v.capacity}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-body font-light text-[11px] text-warmgrey tracking-[0.05em]">{t('vehicleFields.luggage')}</span>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-16">
          <Reveal variant="up"><div>
            ...
            <h2 className="display text-[28px] md:text-[38px] mb-6">
              {content.journeyHeading.line1} <br />
              <span className="display-italic">{content.journeyHeading.italic}</span>
            </h2>
          ...
          <div className="flex flex-col gap-0">
            {faqs.map((faq, i) => (
              <div key={faq.q} className={`py-7 border-b border-anthracite-light ${i === 0 ? 'border-t' : ''}`}>
                ...
                <p className="body-text text-[12px]" style={{ lineHeight: '1.9' }}>{faq.aBidi}</p>
              </div>
            ))}
          </div>
        </div>
