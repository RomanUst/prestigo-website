...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-leipzig', locale)
  const route = await getRoutePrice('prague-leipzig')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-leipzig', {
    indexable: true,
    content: { kind: 'route', key: 'prague-leipzig' },
    locale,
  })
  return {
...
export default async function PragueLeipzigPage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-leipzig', {
            locale,
  ...
  return (
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0"><Image src="/photohero.jpg" alt="Leipzig — private chauffeur transfer from Prague to Leipzig" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} /></div>
