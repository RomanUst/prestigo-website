...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-passau', locale)
  const route = await getRoutePrice('prague-passau')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-passau', {
    indexable: true,
    content: { kind: 'route', key: 'prague-passau' },
    locale,
  })
  return {
...
export default async function PraguePassauPage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-passau', {
            locale,
  ...
  return (
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0"><Image src="/photohero.jpg" alt="Passau — private chauffeur transfer from Prague to Passau" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} /></div>
