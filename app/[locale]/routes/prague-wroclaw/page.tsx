...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-wroclaw', locale)
  const route = await getRoutePrice('prague-wroclaw')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-wroclaw', {
    indexable: true,
    content: { kind: 'route', key: 'prague-wroclaw' },
    locale,
  })
  return {
...
export default async function PragueWroclawPage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-wroclaw', {
            locale,
