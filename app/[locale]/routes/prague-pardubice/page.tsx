...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-pardubice', locale)
  const route = await getRoutePrice('prague-pardubice')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-pardubice', {
    indexable: true,
    content: { kind: 'route', key: 'prague-pardubice' },
    locale,
  })
  return {
...
export default async function PraguePardubicePage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-pardubice', {
            locale,
    ...
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0">
          <Image src="/photohero.jpg" alt="Pardubice — private chauffeur transfer from Prague to Pardubice" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} />
        </div>
