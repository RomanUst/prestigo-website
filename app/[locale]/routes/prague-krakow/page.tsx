...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-krakow', locale)
  const route = await getRoutePrice('prague-krakow')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-krakow', {
    indexable: true,
    content: { kind: 'route', key: 'prague-krakow' },
    locale,
  })
  return {
...
export default async function PragueKrakowPage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-krakow', {
            locale,
  ...
  return (
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0"><Image src="/photohero.jpg" alt="Kraków — private chauffeur transfer from Prague to Kraków" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} /></div>
