...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-hradec-kralove', locale)
  const route = await getRoutePrice('prague-hradec-kralove')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-hradec-kralove', {
    indexable: true,
    content: { kind: 'route', key: 'prague-hradec-kralove' },
    locale,
  })
  return {
...
export default async function PragueHradecKralovePage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-hradec-kralove', {
            locale,
