...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-munich', locale)
  const route = await getRoutePrice('prague-munich')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-munich', {
    indexable: true,
    content: { kind: 'route', key: 'prague-munich' },
    locale,
  })
  return {
...
export default async function PragueMunichPage() {
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route
        ? buildRouteJsonLd(route, 'prague-munich', {
            locale,
  ...
  return (
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0"><Image src="/photohero.jpg" alt="Munich — private chauffeur transfer from Prague to Munich" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} /></div>
