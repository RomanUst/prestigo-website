...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = getRouteContent('prague-plzen', locale)
  const route = await getRoutePrice('prague-plzen')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  const prices = { ePrice }
  const alternates = getAlternates('/routes/prague-plzen', {
    indexable: true,
    content: { kind: 'route', key: 'prague-plzen' },
    locale,
  })
  return {
    ...
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0">
          <Image src="/photohero.jpg" alt="Plzeň — private chauffeur transfer from Prague to Plzeň" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} />
        </div>
