...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-to-strasbourg')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: { absolute: 'Private Transfer Prague to Strasbourg — PRESTIGO Chauffeur' },
    description: `Long-distance private chauffeur transfer from Prague to Strasbourg. 715 km, approx 7.5 hours. Mercedes E, V, or S-Class. From €${ePrice}.`,
    alternates: {
    ...
    openGraph: {
      ...
      description: 'Long-distance private transfer from Prague to Strasbourg. 715 km door-to-door in Mercedes. Quote on request.',
    },
  }
}
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Prague to Strasbourg', item: 'https://rideprestigo.com/routes/prague-to-strasbourg' },
  ],
}

export default async function PragueToStrasbourgPage() {
  const route = await getRoutePrice('prague-to-strasbourg')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
      ...
      <section className="bg-anthracite py-16 md:py-24">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <p className="body-text text-[14px] mb-8" style={{ lineHeight: '1.9' }}>
            A 715-kilometre drive is not the typical PRESTIGO booking — most of our route pages cover destinations within a 5-hour radius of Prague. But our fleet is certified for international travel, our chauffeurs hold EU professional licences, and we run the Prague–Strasbourg route on request throughout the year.
          </p>
          ...
          <p className="body-text text-[13px] mb-6" style={{ lineHeight: '1.9' }}>
            Because 715 km is a 7.5-hour drive, every booking is priced and confirmed individually. The factors that change the final quote include:
          </p>
      ...
      <section className="bg-anthracite-mid py-16 border-t border-anthracite-light">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <h2 className="display text-[24px] md:text-[32px] mb-4">
            Get a quote for Prague to <span className="display-italic">Strasbourg.</span>
          </h2>
