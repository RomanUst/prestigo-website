...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-to-innsbruck')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: { absolute: 'Private Transfer Prague to Innsbruck — PRESTIGO Chauffeur' },
    description: `Long-distance private chauffeur transfer from Prague to Innsbruck. 545 km, approx 6 hours. Mercedes E, V, or S-Class. From €${ePrice}.`,
    alternates: {
    ...
    openGraph: {
      ...
      description: 'Long-distance private transfer from Prague to Innsbruck. 545 km door-to-door in Mercedes. Quote on request.',
    },
  }
}
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Prague to Innsbruck', item: 'https://rideprestigo.com/routes/prague-to-innsbruck' },
  ],
}

export default async function PragueToInnsbruckPage() {
  const route = await getRoutePrice('prague-to-innsbruck')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
      ...
      <section className="bg-anthracite pt-32 pb-16 md:pt-40 md:pb-20 border-b border-anthracite-light">
        <div className="max-w-4xl mx-auto px-6 md:px-12">
          ...
          <h1 className="display text-[36px] md:text-[52px] leading-tight">
            Prague to Innsbruck.<br />
            <span className="display-italic">Private chauffeur.</span>
          </h1>
          <div className="flex flex-wrap gap-8 mt-8">
            <div>
              <p className="label mb-1">Distance</p>
              <p className="font-body font-light text-[14px] text-offwhite">545 km</p>
            </div>
            <div>
              <p className="label mb-1">Duration</p>
              <p className="font-body font-light text-[14px] text-offwhite">approx. 6 hours</p>
            </div>
            <div>
      ...
      <section className="bg-anthracite py-16 md:py-24">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <p className="body-text text-[14px] mb-8" style={{ lineHeight: '1.9' }}>
            A 545-kilometre drive is not the typical PRESTIGO booking — most of our route pages cover destinations within a 5-hour radius of Prague. But our fleet is certified for international travel, our chauffeurs hold EU professional licences, and we run the Prague–Innsbruck route on request throughout the year.
          </p>
          ...
          <p className="body-text text-[13px] mb-6" style={{ lineHeight: '1.9' }}>
            Because 545 km is a 6-hour drive, every booking is priced and confirmed individually. The factors that change the final quote include:
          </p>
      ...
      <section className="bg-anthracite-mid py-16 border-t border-anthracite-light">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <h2 className="display text-[24px] md:text-[32px] mb-4">
            Get a quote for Prague to <span className="display-italic">Innsbruck.</span>
          </h2>
