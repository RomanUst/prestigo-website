...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-zlin')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: `Prague to Zlín Chauffeur — From €${ePrice}`,
    description: `Book a private chauffeur from Prague to Zlín. 310 km door-to-door in a Mercedes-Benz. Fixed price from €${ePrice}, Baťa's functionalist city.`,
    alternates: {
    ...
    openGraph: {
      ...
      title: `Prague to Zlín Private Transfer — From €${ePrice}`,
      description: `Book a private chauffeur from Prague to Zlín. 310 km door-to-door in a Mercedes-Benz. Fixed price from €${ePrice}, Baťa's functionalist city.`,
...
export default async function PragueZlinPage() {
  const route = await getRoutePrice('prague-zlin')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  ...
  return (
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0"><Image src="/photohero.png" alt="Zlín — private chauffeur transfer from Prague to Zlín" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} /></div>
      ...
      <section className="bg-anthracite py-20">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <Reveal variant="up"><div><h2 className="display text-[28px] md:text-[36px]">Prague to Zlín.<br /><span className="display-italic">From €{ePrice}, fixed.</span></h2><p className="body-text text-[13px] mt-4">No surprises. No meters. Your driver is waiting.</p></div></Reveal>
