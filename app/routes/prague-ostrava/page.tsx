...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-ostrava')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: `Prague to Ostrava Chauffeur — From €${ePrice}`,
    description: `Book a private chauffeur from Prague to Ostrava. 370 km door-to-door in a Mercedes-Benz. Fixed price from €${ePrice}, Moravian-Silesian capital.`,
    alternates: {
    ...
    openGraph: {
      ...
      title: `Prague to Ostrava Private Transfer — From €${ePrice}`,
      description: `Book a private chauffeur from Prague to Ostrava. 370 km door-to-door in a Mercedes-Benz. Fixed price from €${ePrice}, Moravian-Silesian capital.`,
...
export default async function PragueOstravaPage() {
  const route = await getRoutePrice('prague-ostrava')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  ...
  const faqs = [
    ...
    { q: 'How much does a chauffeur from Prague to Ostrava cost?', a: `Fixed fare from €${ePrice} in Mercedes E-Class (up to 3 passengers), €${vPrice} in V-Class (up to 6 passengers), or €${sPrice} in S-Class. Prices include fuel, the Czech motorway vignette, driver time, and every toll. No meter, no surge, no hidden charges.` },
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...(route ? buildRouteJsonLd(route, 'prague-ostrava')['@graph'] : []),
      {
        '@type': 'FAQPage',
  ...
  return (
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0"><Image src="/photohero.jpg" alt="Ostrava — private chauffeur transfer from Prague to Ostrava" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} /></div>
