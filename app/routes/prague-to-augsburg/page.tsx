...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-to-augsburg')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: { absolute: 'Private Transfer Prague to Augsburg — PRESTIGO Chauffeur' },
    description: `Long-distance private chauffeur transfer from Prague to Augsburg. 430 km, approx 5 hours. Mercedes E, V, or S-Class. From €${ePrice}.`,
    alternates: {
...
export default async function PragueToAugsburgPage() {
  const route = await getRoutePrice('prague-to-augsburg')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
