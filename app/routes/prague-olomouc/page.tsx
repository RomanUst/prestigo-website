...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-olomouc')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: `Prague to Olomouc Chauffeur — From €${ePrice}`,
    description: `Book a private chauffeur from Prague to Olomouc. 280 km door-to-door in a Mercedes-Benz. Fixed price from €${ePrice}, Moravia's historic capital.`,
    alternates: {
    ...
    openGraph: {
      ...
      title: `Prague to Olomouc Private Transfer — From €${ePrice}`,
      description: `Book a private chauffeur from Prague to Olomouc. 280 km door-to-door in a Mercedes-Benz. Fixed price from €${ePrice}, Moravia's historic capital.`,
...
export default async function PragueOlomoucPage() {
  const route = await getRoutePrice('prague-olomouc')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  ...
  const faqs = [
    ...
    { q: 'How much does a chauffeur from Prague to Olomouc cost?', a: `Fixed fare from €${ePrice} in Mercedes E-Class (up to 3 passengers), €${vPrice} in V-Class (up to 6 passengers), or €${sPrice} in S-Class. Prices include fuel, the Czech motorway vignette, and driver time. No hidden charges.` },
    { q: 'Can I book a same-day round trip from Prague to Olomouc?', a: 'Yes. You can book the journey there and back with a 10% same-day return discount. If you need the chauffeur to move around Olomouc with you during the visit, add hourly city rental to the booking. Most clients book a full day to cover Upper Square, the Holy Trinity Column, and lunch in the old town before the drive back.' },
  ...
  const pageSchema = {
    ...
    '@graph': [
      ...
      {
        '@type': 'BreadcrumbList',
        '@id': 'https://rideprestigo.com/routes/prague-olomouc#breadcrumb',
        itemListElement: [
          { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://rideprestigo.com' },
          { '@type': 'ListItem', position: 2, name: 'Routes', item: 'https://rideprestigo.com/routes' },
          { '@type': 'ListItem', position: 3, name: 'Prague to Olomouc', item: 'https://rideprestigo.com/routes/prague-olomouc' },
        ],
      },
    ],
  }
  ...
  return (
    <main id="main-content">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '560px' }}>
        <div className="absolute inset-0"><Image src="/photohero.png" alt="Olomouc — private chauffeur transfer from Prague to Olomouc" fill priority sizes="100vw" className="object-cover" style={{ filter: 'brightness(0.38)' }} /></div>
      ...
      <section className="bg-anthracite py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-16">
          <Reveal variant="up"><div>
            ...
            <h2 className="display text-[28px] md:text-[38px] mb-6">Prague to Olomouc,<br /><span className="display-italic">the route.</span></h2>
      ...
      <section className="bg-anthracite py-20">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <Reveal variant="up"><div><h2 className="display text-[28px] md:text-[36px]">Prague to Olomouc.<br /><span className="display-italic">From €{ePrice}, fixed.</span></h2><p className="body-text text-[13px] mt-4">No surprises. No meters. Your driver is waiting.</p></div></Reveal>
