...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-to-erfurt')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: { absolute: 'Private Transfer Prague to Erfurt — PRESTIGO Chauffeur' },
    description: `Long-distance private chauffeur transfer from Prague to Erfurt. 250 km, approx 3 hours. Mercedes E, V, or S-Class. From €${ePrice}.`,
    alternates: {
    ...
    openGraph: {
      ...
      description: 'Long-distance private transfer from Prague to Erfurt. 250 km door-to-door in Mercedes. Quote on request.',
    },
  }
}
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Prague to Erfurt', item: 'https://rideprestigo.com/routes/prague-to-erfurt' },
  ],
}

export default async function PragueToErfurtPage() {
  const route = await getRoutePrice('prague-to-erfurt')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
        ...
        <div className="max-w-4xl mx-auto px-6 md:px-12">
          ...
          <div className="flex flex-wrap gap-8 mt-8">
            <div>
              ...
              <p className="font-body font-light text-[14px] text-offwhite">250 km</p>
            </div>
            <div>
              <p className="label mb-1">Duration</p>
              <p className="font-body font-light text-[14px] text-offwhite">approx. 3 hours</p>
            </div>
            <div>
      ...
      <section className="bg-anthracite py-16 md:py-24">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <p className="body-text text-[14px] mb-8" style={{ lineHeight: '1.9' }}>
            A 250-kilometre drive is not the typical PRESTIGO booking — most of our route pages cover destinations within a 5-hour radius of Prague. But our fleet is certified for international travel, our chauffeurs hold EU professional licences, and we run the Prague–Erfurt route on request throughout the year.
          </p>
          ...
          <p className="body-text text-[13px] mb-6" style={{ lineHeight: '1.9' }}>
            Because 250 km is a 3-hour drive, every booking is priced and confirmed individually. The factors that change the final quote include:
          </p>
          <ul className="flex flex-col gap-4 mb-8">
            {[
              ...
              'Time of year. Winter routes may add time for weather conditions.',
              'Overnight requirements. EU driver regulation 561/2006 limits continuous driving to 4.5 hours before a 45-minute break, and 9 hours total in a day.',
              'Return or one-way. Same-day return on this distance is possible but unusual; most bookings are one-way.',
          ...
          <ul className="flex flex-col gap-4 mb-10">
            {[
              'A black Mercedes — E-Class, S-Class, or V-Class depending on group size and preference.',
      ...
      <section className="bg-anthracite-mid py-16 border-t border-anthracite-light">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <h2 className="display text-[24px] md:text-[32px] mb-4">
            Get a quote for Prague to <span className="display-italic">Erfurt.</span>
          </h2>
