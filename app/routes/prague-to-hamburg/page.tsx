...
export async function generateMetadata(): Promise<Metadata> {
  const route = await getRoutePrice('prague-to-hamburg')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
  return {
    title: { absolute: 'Private Transfer Prague to Hamburg — PRESTIGO Chauffeur' },
    description: `Long-distance private chauffeur transfer from Prague to Hamburg. 680 km, approx 7 hours. Mercedes E, V, or S-Class. From €${ePrice}.`,
    alternates: {
    ...
    openGraph: {
      ...
      description: 'Long-distance private transfer from Prague to Hamburg. 680 km door-to-door in Mercedes. Quote on request.',
    },
  }
}
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Prague to Hamburg', item: 'https://rideprestigo.com/routes/prague-to-hamburg' },
  ],
}

export default async function PragueToHamburgPage() {
  const route = await getRoutePrice('prague-to-hamburg')
  const ePrice = route?.eClassEur ?? ROUTE_FALLBACK.eClassEur
      ...
      <section className="bg-anthracite py-16 md:py-24">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <p className="body-text text-[14px] mb-8" style={{ lineHeight: '1.9' }}>
            A 680-kilometre drive is not the typical PRESTIGO booking — most of our route pages cover destinations within a 5-hour radius of Prague. But our fleet is certified for international travel, our chauffeurs hold EU professional licences, and we run the Prague–Hamburg route on request throughout the year.
          </p>
          ...
          <p className="body-text text-[13px] mb-6" style={{ lineHeight: '1.9' }}>
            Because 680 km is a 7-hour drive, every booking is priced and confirmed individually. The factors that change the final quote include:
          </p>
      ...
      <section className="bg-anthracite-mid py-16 border-t border-anthracite-light">
        <div className="max-w-3xl mx-auto px-6 md:px-12">
          <h2 className="display text-[24px] md:text-[32px] mb-4">
            Get a quote for Prague to <span className="display-italic">Hamburg.</span>
          </h2>
